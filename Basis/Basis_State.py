...
# @Last Modified time: 2023-03-27 10:14:20
...
class Mea_basis(State):
    ...
    def Get_basis(self):
        ...
        elif self.basis == 'Pauli_4':
            self.K = 4
            Ps = 1.0 / 3.0
            self.M = np.zeros((self.K, 2, 2), dtype=np.complex64)
            self.M[0, :, :] = Ps * np.array([[1, 0], [0, 0]])
            self.M[1, :, :] = Ps * np.array([[0, 0], [0, 1]])
            self.M[2, :, :] = Ps / 2 * np.array([[1, 1], [1, 1]])
            self.M[3, :, :] = Ps / 2 * (np.array([[1, -1], [-1, 1]]) + 
                                        np.array([[1, -1j], [1j, 1]]) + 
                                        np.array([[1, 1j], [-1j, 1]]))
...
class MPS_state():
    def __init__(self, MPS='GHZ', N=2):
        if MPS == 'GHZ':
            ...
            self.MPS = readMPS(MPSf=MPS, N=N, convert=True)
...
if __name__ == '__main__':
    B = Mea_basis('Psi2')  # Tetra, 4Pauli, Pauli, Trine, Psi2
    #print(B.K, B.M)
    B.Basis_info()
