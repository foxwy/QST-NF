...
# @Last Modified time: 2023-03-30 16:44:59
...
def samples_mp(param):
    ...
    idxs = np.argmax(np.random.multinomial(1, P_all, group_N), 1)
    S_all = []
    S_one_hot_all = []
    ...
    S_one_hot_all = np.reshape(np.eye(K)[S_all], [group_N, N * K]).astype(np.uint8)
