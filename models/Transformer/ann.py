...
class Transformer(nn.Module):
    ...
    def __init__(self, decoder, tgt_embed, generator, Nq, Na, device):
    ...
    def p(self, a_mat_all):
        P_all = []
        N_all = len(a_mat_all)
        for k in range(int(N_all / 10**3)):
            a_mat = a_mat_all[k * 10**3:(k + 1) * 10**3]

            outcome = a_mat + 3
            N_s = len(a_mat)
            trg = torch.tensor(np.c_[np.array([1]*N_s), outcome]).to(torch.long).to(self.device)
            ...
            for i in range(N_s):
                ...
                for nq in range(self.Nq):
                    p *= ((p_tensor[i, nq, 3:] / sum(p_tensor[i, nq, 3:]))[a_mat[i, nq]]).item()
                P_all.append(p)
    ...
    def generate_next(self, a_vec):
        ...
        for a_ind in range(len(a_vec)):
            a_vec[a_ind] += 3
        trg = torch.tensor([[1] + a_vec]).to(torch.long).to(self.device)
        trg_mask = Batch.make_std_mask(trg, 0)
        out = self.forward(trg, trg_mask)
        log_p = self.generator(out)
        p_tensor = torch.exp(log_p)

        p_vec = p_tensor[0, -1].cpu().detach().numpy()[3:]
...
@torch.no_grad()
def sample_mp(model, Ns, group_N, device):
    model.to(device)
    Nq = model.Nq

    if Ns < group_N:
        group_N = Ns
    groups = [group_N] * (Ns // group_N)
    if Ns % group_N != 0:
        groups.append(Ns % group_N)

    samples = np.ones((Ns, Nq))
    Ps = np.ones(Ns)
    for idx, num in enumerate(groups):
        print('sampling num', idx * group_N + num)
        outcomes = torch.ones(num, Nq+1, dtype=int).to(device)
        P = torch.ones(num).to(device)
        for i in range(Nq):
            log_p = model.generator(model.forward(
                outcomes[:, :i+1], Batch.make_std_mask(outcomes[:, :i+1], 0)))
            p_tensor = torch.exp(log_p)[:, -1, 3:]
            p_tensor = p_tensor / torch.sum(p_tensor, dim=1).reshape((num, 1))

            a_tmp = torch.multinomial(p_tensor, 1)
            P *= torch.sum(F.one_hot(a_tmp.reshape(-1), p_tensor.size(1)) * p_tensor, dim=1)
            next_a = a_tmp + 3
            outcomes[:, i+1] = next_a.reshape(num)
        samples[idx * group_N: idx * group_N + num, :] = outcomes.cpu().numpy()[:, 1:] - 3
        Ps[idx * group_N: idx * group_N + num] = P.cpu().numpy()

    return samples.astype(np.uint8), Ps
...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, x, target):
        ...
        true_dist.scatter_(1, target.data.unsqueeze(
            1).type(torch.int64), self.confidence)
...
def data_to_torch(data):
    ...
    return torch.from_numpy(data_np).to(torch.long)
...
def make_model(Nq, tgt_vocab, N=6, d_model=512, d_ff=2048, h=8, dropout=0., device='cuda:0'):
    ...
    model = Transformer(
        ...
        Generator(d_model, tgt_vocab), Nq, tgt_vocab-3, device=device)
...
# / ONE EPOCH
def InitializeModel(Nq, Nlayer=2, dmodel=128, Nh=4, Na=4, dropout=0., device='cuda:0'):
    V = Na + 3
    # Initialize Model
    model = make_model(Nq=Nq, tgt_vocab=V, N=Nlayer,
                       d_model=dmodel, d_ff=4*dmodel, h=Nh, dropout=dropout, device=device)

    return model


def TrainModel(model, data_np, device, smoothing=0.0, lr=0.001, batch_size=100, Nep=20):
    V = model.Na + 3
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=Nep, eta_min=0.)

    if len(data_np) < batch_size:
        batch_size = len(data_np)
    data = data_to_torch(data_np).to(device)

    pbar = tqdm(range(Nep))
    for k in pbar:
        model.train()
        loss[k] = run_epoch(
            data_gen(data, batch_size),
            model,
            SimpleLossCompute(model.generator, criterion, optimizer),
            verbose=False)
        scheduler.step()

        pbar.set_description("loss {:.6f}".format(loss[k]))
    pbar.close()
