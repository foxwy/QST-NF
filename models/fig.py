...
# @Last Modified by:   WY
# @Last Modified time: 2022-11-08 15:58:39
...
if __name__ == '__main__':
    ...
    for i in N_qubit:
        ...
        #-----DM-----
        fdm_sample = 'DDM/samples/' + state_name + '_' + povm + '_samples_N' + str(i) + '_sample.txt'
        fdm_P = 'DDM/samples/' + state_name + '_' + povm + '_samples_N' + str(i) + '_P.txt'
        ...
        #-----RNN----
        frnn_sample = 'RNN/samples/' + state_name + '_' + povm + '_samples_N' + str(i) + '_sample.txt'
        frnn_P = 'RNN/samples/' + state_name + '_' + povm + '_samples_N' + str(i) + '_P.txt'
        ...
        #-----Transformer----
        attn_sample = 'Transformer/samples/' + state_name + '_' + povm + '_samples_N' + str(i) + '_sample.txt'
        attn_P = 'Transformer/samples/' + state_name + '_' + povm + '_samples_N' + str(i) + '_P.txt'
    ...
    '''
    i = 5
    fdm_sample = 'DDM/samples/' + 'GHZ_MPS' + '_' + 'Tetra4' + '_samples_N' + str(i) + '.txt'
    dm_sample = np.loadtxt(fdm_sample).astype(np.uint8)
    idxs_dm = dm_sample.dot(4**(np.arange(i - 1, -1, -1)))
    weights_dm = np.ones_like(idxs_dm) / float(len(idxs_dm))

    frnn_sample = 'RNN/samples/' + 'GHZ_MPS' + '_' + 'Tetra4' + '_samples_N' + str(i) + '.txt'
    rnn_sample = np.loadtxt(frnn_sample).astype(np.uint8)
    idxs_rnn = rnn_sample.dot(4**(np.arange(i - 1, -1, -1)))
    weights_rnn = np.ones_like(idxs_rnn) / float(len(idxs_rnn))

    trainFileName = filepath + '/datasets/data/' + 'GHZ_MPS' + '_' + 'Tetra4' + '_data_N' + str(i) + '.txt'
    data_train = np.loadtxt(trainFileName).astype(int)
    idxs_o = data_train.dot(4**(np.arange(i - 1, -1, -1)))
    weights_o = np.ones_like(idxs_o) / float(len(idxs_o))

    trainFileName = filepath + '/datasets/data/' + 'GHZ_MPS' + '_' + 'Tetra4' + '_data_N_t' + str(i) + '.txt'
    data_train = np.loadtxt(trainFileName).astype(int)
    idxs_t = data_train.dot(4**(np.arange(i - 1, -1, -1)))
    weights_t = np.ones_like(idxs_t) / float(len(idxs_t))

    bins = np.linspace(0, 4**i, 80)
    fig, ax = plt.subplots(1, 1, figsize=(15, 4))
    ax.hist([idxs_dm, idxs_rnn, idxs_o, idxs_t][::-1], bins, weights=[weights_dm, weights_rnn, weights_o, weights_t][::-1], \
             label=['VDM', 'RNN', 'Original', 'True'][::-1], color=['c', 'orange', 'blue', 'red'][::-1])
    ...
    ax.legend(prop=font2, frameon=True)
    ax.set_xlabel('Measurement', font)  # fontweight='bold'
    ax.set_ylabel("Probabilities", font)
    plt.savefig('figure/DS.pdf', dpi=600, bbox_inches='tight', pad_inches=0.05)
    plt.show()'''
    ...
    for N in N_s:
        ...
        #-----VDM-----
        fdm_sample = 'DDM/samples/' + state_name + '_' + povm + '_samples_N' + str(N_q) + '_' + str(int(N/1000)) + '_sample.txt'
        fdm_P = 'DDM/samples/' + state_name + '_' + povm + '_samples_N' + str(N_q) + '_' + str(int(N/1000)) + '_P.txt'
        dm_sample = np.loadtxt(fdm_sample).astype(np.uint8)
        dm_P = np.loadtxt(fdm_P)
        ...
        #----RNN-----
        frnn_sample = 'RNN/samples/' + state_name + '_' + povm + '_samples_N' + str(N_q) + '_' + str(int(N/1000)) + '_sample.txt'
        frnn_P = 'RNN/samples/' + state_name + '_' + povm + '_samples_N' + str(N_q) + '_' + str(int(N/1000)) + '_P.txt'
        rnn_sample = np.loadtxt(frnn_sample).astype(np.uint8)
        rnn_P = np.loadtxt(frnn_P)
        ...
        ##----RNN-----
        fattn_sample = 'Transformer/samples/' + state_name + '_' + povm + '_samples_N' + str(N_q) + '_' + str(int(N/1000)) + '_sample.txt'
        fattn_P = 'Transformer/samples/' + state_name + '_' + povm + '_samples_N' + str(N_q) + '_' + str(int(N/1000)) + '_P.txt'
